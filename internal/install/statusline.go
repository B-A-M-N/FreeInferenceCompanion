...
func installerLockPathForScope(base string, scope InstallScope, projectRoot string) string {
	...
	default:
		return filepath.Join(projectRoot, ".claude", "fi-statusline.lock")
	}
}
...
//
// P0-3: when fi produces no output (inactive or ineligible), the wrapper
// must not emit a blank line — the user's existing status line runs
// standalone. Only emit the previous output when fi actually produced
