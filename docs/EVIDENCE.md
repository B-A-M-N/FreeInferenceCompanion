...
| Codex | 0.149.0 | Historical evidence only; see the limitation below until a current HarvardCodex capture replaces it |
...
The historical Codex screenshot predates rollout-backed reporting and is not
evidence of current user-visible behavior. Current releases provide verified
session-bound rollout counters through the CLI and `codex-surface render`; a
real current HarvardCodex capture is required before release evidence claims a
rendered rich-line result. Server-side cache policy and compaction effectiveness remain
