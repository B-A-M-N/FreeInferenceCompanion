...
func TestCodexMarketplaceEntryMatchesPlugin(t *testing.T) {
	root := filepath.Dir(pluginRoot())
	...
	if marketplace.Name != "freeinference-companion" || len(marketplace.Plugins) != 1 {
		t.Fatalf("unexpected marketplace: %+v", marketplace)
	}
	entry := marketplace.Plugins[0]
	if entry.Name != "freeinference-companion" || entry.Source.Type != "local" || entry.Source.Path != "./plugins/freeinference-companion" {
		t.Errorf("invalid Codex marketplace entry: %+v", entry)
	}
	pluginManifest := filepath.Join(root, strings.TrimPrefix(entry.Source.Path, "./"), ".codex-plugin", "plugin.json")
...
func TestCodexVersionIsSemver(t *testing.T) {
	pj := filepath.Join(pluginDir("freeinference-companion"), ".codex-plugin", "plugin.json")
	data, err := os.ReadFile(pj)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCodexSkillsDirectoryValid(t *testing.T) {
	skillsDir := filepath.Join(pluginDir("freeinference-companion"), "skills")
	info, err := os.Stat(skillsDir)
	if err != nil {
		t.Fatalf("skills directory: %v", err)
	}
